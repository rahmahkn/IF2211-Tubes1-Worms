...
public class Bot {
    ...
    public Command run() {
        ...
        if(euclideanDistance(enemy2.position.x,enemy2.position.y,currentWorm.position.x,currentWorm.position.y)>currentWorm.weapon.range){
            //samperin enemy2 nya
            Cell nearEnemy = getRandomAdjacentCellEnemy(enemy2.position);
            return digAndMoveTo(nearEnemy.x,nearEnemy.y);
        }
        */
        //follow cacing lain
        ...
        if(euclideanDistance(cacinglaen.position.x,cacinglaen.position.y,currentWorm.position.x,currentWorm.position.x)>=4
                && cacinglaen.health>0 ){ 
